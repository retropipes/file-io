...
public class JsonException extends Exception{
	 ...
	 * @param nposition where the exception occurred.
	 * @param nProblemType how the exception occurred.
	 * @param nUnexpectedObject what caused the exception. */
	public JsonException(final long nposition, final Problems nProblemType, final Object nUnexpectedObject){
		this.position = nposition;
		this.problemType = nProblemType;
		this.unexpectedObject = nUnexpectedObject;
		if(Problems.IOEXCEPTION.equals(nProblemType) || Problems.UNEXPECTED_EXCEPTION.equals(nProblemType)){
			if(nUnexpectedObject instanceof Throwable){
				this.initCause((Throwable)nUnexpectedObject);
			}
		}
	}
