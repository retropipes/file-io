...
public class JDataWriter implements DataIOWriter {
    ...
    // Constructors
    public JDataWriter(final String filename, final String newDocTag) throws IOException {
    ...
    public JDataWriter(final File filename, final String newDocTag) throws IOException {
	this.fileIO = new BufferedWriter(new FileWriter(filename));
	this.file = filename;
	this.docTag = newDocTag;
	this.writeXHeader();
    ...
    public JDataWriter(final OutputStream stream, final String newDocTag) throws IOException {
	...
	try {
	    this.fileIO.write("<" + XDataConstants.STRING_TAG + ">" //$NON-NLS-1$ //$NON-NLS-2$
		    + JDataWriter.replaceSpecialCharacters(s) + "</" //$NON-NLS-1$
		    + XDataConstants.STRING_TAG + ">" + JDataWriter.END_OF_LINE); //$NON-NLS-1$
	} catch (IOException e) {
	    throw new DataIOException(e);
	}
    }
	...
	try {
	    this.fileIO.write("<" + JDataWriter.replaceSpecialCharacters(groupName) + ">" + JDataWriter.END_OF_LINE);
	} catch (IOException e) {
	    throw new DataIOException(e);
	}
    }
	...
	try {
	    this.fileIO.write(XDataConstants.X_HEADER + JDataWriter.END_OF_LINE);
	} catch (IOException e) {
	    throw new DataIOException(e);
	}
    }
	...
	try {
	    this.fileIO.write("<" + this.docTag + ">" + JDataWriter.END_OF_LINE); //$NON-NLS-1$ //$NON-NLS-2$
	} catch (IOException e) {
	    throw new DataIOException(e);
	}
    }
